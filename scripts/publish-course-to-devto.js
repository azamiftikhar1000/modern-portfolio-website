...
function getBanner(section, course_slug, slug) {
  return `https://raw.githubusercontent.com/azamiftikhar/portfolio/master/public/static/courses/${course_slug}/${section}/${slug}/banner.png`;
}
...
function getBody(course_slug, slug) {
  ...
  body = body.replace(
    /\]\(\/static\/courses/g,
    '](https://raw.githubusercontent.com/azamiftikhar/portfolio/master/public/static/courses',
  );

  // Replace static links
  body = body.replace(
    new RegExp(`]\\(/courses/${course_slug}`, 'g'),
    `](https://azamiftikhar.dev/courses/${course_slug}`,
  );
...
_This article is part of my open-source [System Design Course](https://github.com/azamiftikhar1000/system-design) available on Github._

{% github azamiftikhar/system-design %}`;
...
(async function main() {
  ...
  for (const [, { name, section, slug }] of articles.entries()) {
    ...
    const canonical_url = `https://github.com/azamiftikhar1000/${course_slug}#${slug}`;
